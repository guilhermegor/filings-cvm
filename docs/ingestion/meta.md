...
[portal de dados abertos](https://dados.cvm.gov.br/dados/). Um reader por dataset, **48 no total**,
...
| `FI/DOC/EVENTUAL` | `MetaEventualFiReader` | `.txt` (11 campos) |
| `FI/DOC/PERFIL_MENSAL` | `MetaPerfilMensalFiReader` | `.txt` (107 campos — só o regime pós-RCVM 175) |
| `FI/DOC/EXTRATO` | `MetaExtratoFiReader` | `.txt` (117 campos — só o schema de 2020 em diante) |
...
| `CIA_ABERTA/DOC/IPE` | `MetaIpeCiaAbertaReader` | `.txt` |
| `CIA_ABERTA/DOC/VLMO` | `MetaVlmoCiaAbertaReader` | `.zip` |
| `CIA_ABERTA/DOC/FCA` | `MetaFcaCiaAbertaReader` | `.zip` ⚠️ **sem o prefixo `meta_`** |
| `CIA_ABERTA/DOC/CGVN` | `MetaCgvnCiaAbertaReader` | `.zip` |
| `CIA_ABERTA/DOC/FRE` | `MetaFreCiaAbertaReader` | `.zip` |
| `CIA_ABERTA/DOC/DFP` | `MetaDfpCiaAbertaReader` | `.zip` (infixo `_txt`) |
| `CIA_ABERTA/DOC/ITR` | `MetaItrCiaAbertaReader` | `.zip` (infixo `_txt`) |
| `CIA_ABERTA/EVENTOS/RECOMPRA_ACOES` | `MetaRecompraAcoesReader` | `.zip` |

> ℹ️ **`FIE/MEDIDAS`** publica `.csv` **e** `.txt`; usamos o **`.txt`**, o formato que os 42
> compartilham.
