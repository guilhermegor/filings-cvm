"""Unit tests for the two COORD_OFERTA/CAD registry readers (`cad_coord_oferta.zip`).

One parameterized module covers both members: a single in-memory fixture ZIP holds the
`coord_oferta` and `resp` CSVs, and each reader is asserted to select **its own** member, coerce
its date columns, and honour the shared base behaviour (no ``date_ref``, ``path_raw`` persistence,
contract validation). The one non-tautological assertion is
``test_contract_matches_the_published_header``: it compares each contract against the verbatim
header bytes CVM publishes, committed under ``tests/fixtures/cad_coord_oferta/`` — the pinned
...
from filings_cvm._internal.config.contracts import CAD_COORD_OFERTA, CAD_COORD_OFERTA_RESP
from filings_cvm._internal.config.ports.ingestion_reader import IngestionReader
from filings_cvm._internal.utils.tabular_reader import ContractError, FileContract
from filings_cvm.ingestion.coord_oferta import CoordOfertaReader, CoordOfertaRespReader
...
PATH_FIXTURES = Path(__file__).resolve().parents[1] / "fixtures" / "cad_coord_oferta"
...
ALL_READERS: tuple[type[IngestionReader], ...] = (CoordOfertaReader, CoordOfertaRespReader)
...
SECTIONS: tuple[tuple[str, type[IngestionReader], FileContract], ...] = (
	("cad_coord_oferta", CoordOfertaReader, CAD_COORD_OFERTA),
	("cad_coord_oferta_resp", CoordOfertaRespReader, CAD_COORD_OFERTA_RESP),
)
...
def _patch_download(monkeypatch: pytest.MonkeyPatch, payload: bytes) -> None:
	...
	monkeypatch.setattr(
		"filings_cvm.ingestion.coord_oferta.cad._base_coord_oferta_reader.download_file",
		_fake_download,
	)
...
def test_contract_matches_the_published_header(
	...
	----------
	str_section : str
		The member section (``cad_coord_oferta`` / ``cad_coord_oferta_resp``).
...
def test_the_two_members_have_distinct_columns() -> None:
	...
	Both are keyed by the coordinator's ``CNPJ``, but ``resp`` is not a subset of the registry: it
	carries the officer-specific ``TP_RESP``/``RESP``/``DT_INI_RESP`` columns absent from it.
	Copying the sibling would ship the wrong contract, all tests green.
	"""
	set_registry = set(CAD_COORD_OFERTA.tuple_required)
	set_resp = set(CAD_COORD_OFERTA_RESP.tuple_required)
	assert set_resp - set_registry == {"TP_RESP", "RESP", "DT_INI_RESP"}
...
def test_read_selects_its_own_member_with_all_columns(
	...
	This also proves ``find_member`` selects by exact name: ``cad_coord_oferta.csv`` must not match
	the longer ``cad_coord_oferta_resp.csv``.
...
def test_meta_url_is_the_zip_not_a_txt() -> None:
	...
	assert MetaCoordOfertaReader._META_URL.endswith("/META/meta_cad_coord_oferta.zip")


def test_meta_section_labels_are_asymmetric_by_design() -> None:
	"""The META's two ``section`` labels are ``cad_coord_oferta`` and ``resp``, not a tidy pair.

	``meta_cad_coord_oferta.txt`` is named with the bare ``_MEMBER_STEM`` and has no ``<stem>_``
	suffix to strip, so the base's ``_section_of`` falls back to the whole stem; the sibling
	``meta_cad_coord_oferta_resp.txt`` reduces to ``resp``. The same asymmetry INTERMED has. Pinned
	so it stays a recorded source fact rather than something a future change silently "tidies".
	"""
	from filings_cvm.ingestion.coord_oferta.cad.meta import MetaCoordOfertaReader

	cls_reader = MetaCoordOfertaReader()
	assert cls_reader._section_of("meta_cad_coord_oferta.txt") == "cad_coord_oferta"
	assert cls_reader._section_of("meta_cad_coord_oferta_resp.txt") == "resp"


def test_both_members_are_keyed_by_the_coordinator_cnpj() -> None:
	"""Both members constrain the coordinator's ``CNPJ`` — no personal identifier is validated.

	``resp`` carries an officer's name but the source publishes **no** CPF column, so nothing
	person-level is ever CNPJ-validated.
	"""
	assert CAD_COORD_OFERTA.tuple_cnpj_cols == ("CNPJ",)
	assert CAD_COORD_OFERTA_RESP.tuple_cnpj_cols == ("CNPJ",)
	assert not any("CPF" in c for c in CAD_COORD_OFERTA_RESP.tuple_required)
...
def test_read_raises_contract_error_on_missing_required_column(
	...
	list_cols = [c for c in CoordOfertaReader._CONTRACT.tuple_required if c != "DENOM_SOCIAL"]
	dict_members[CoordOfertaReader._MEMBER] = (
		";".join(list_cols) + "\n" + ";".join("x" for _ in list_cols) + "\n"
	)
	_patch_download(monkeypatch, _zip_bytes(dict_members))

	with pytest.raises(ContractError):
		CoordOfertaReader().read()
...
def test_read_raises_value_error_when_member_absent(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	"""
	dict_members = _all_members()
	del dict_members[CoordOfertaRespReader._MEMBER]
	_patch_download(monkeypatch, _zip_bytes(dict_members))

	with pytest.raises(ValueError, match="cad_coord_oferta_resp.csv"):
		CoordOfertaRespReader().read()
...
def test_read_persists_whole_archive_when_path_raw_is_given(
	...
	CoordOfertaReader(path_raw=path_raw).read()

	assert (path_raw / "cad_coord_oferta.zip").is_file()
...
def test_read_rejects_wrong_argument_type(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	with pytest.raises(TypeError):
		CoordOfertaReader().read(int_timeout_s="nope")
