"""Unit tests for the Informe Trimestral FIP reader.

``InfTrimestralFipReader`` reads ``inf_trimestral_fip_AAAA.csv`` — a plain CSV (not a ZIP),
partitioned by **year**, the pre-RCVM 175 quarterly FIP report. ``DT_COMPTC`` is coerced to a
``date``; money and quota amounts stay exact source text. Mock the single I/O boundary
...
from filings_cvm import InfTrimestralFipReader, RetryPolicy
from filings_cvm._internal.config.contracts import INF_TRIMESTRAL_FIP
...
def _valid_row() -> list[str]:
	...
	return [_value_for(str_col) for str_col in INF_TRIMESTRAL_FIP.tuple_required]
...
def _default_csv() -> str:
	...
	return _csv(list(INF_TRIMESTRAL_FIP.tuple_required), [_valid_row()])
...
def test_read_returns_all_contract_columns(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	df_ = InfTrimestralFipReader(date_ref=REF).read()

	assert len(df_) == 1
	assert list(df_.columns) == list(INF_TRIMESTRAL_FIP.output_columns)
...
def test_read_coerces_dt_comptc(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	assert df_["DT_COMPTC"].iloc[0] == date(2023, 3, 31)
...
def test_read_keeps_money_columns_as_text(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	monkeypatch : pytest.MonkeyPatch
		Fixture used to replace the download boundary.
	"""
	list_cols = list(INF_TRIMESTRAL_FIP.tuple_required)
	list_row = _valid_row()
	list_row[list_cols.index("VL_PATRIM_LIQ")] = "4603790.170000000000"
	_patch_download(monkeypatch, _csv(list_cols, [list_row]))

	df_ = InfTrimestralFipReader(date_ref=REF).read()

	assert df_["VL_PATRIM_LIQ"].iloc[0] == "4603790.170000000000"
...
def test_date_ref_selects_the_year(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	InfTrimestralFipReader(date_ref=date(2020, 2, 29)).read()

	assert list_urls == [
		"https://dados.cvm.gov.br/dados/FIP/DOC/INF_TRIMESTRAL/DADOS/inf_trimestral_fip_2020.csv"
	]
...
def test_read_raises_contract_error_on_missing_required_column(
	...
	monkeypatch : pytest.MonkeyPatch
		Fixture used to replace the download boundary.
	"""
	list_cols = [c for c in INF_TRIMESTRAL_FIP.tuple_required if c != "VL_PATRIM_LIQ"]
	list_row = [
		v
		for c, v in zip(INF_TRIMESTRAL_FIP.tuple_required, _valid_row(), strict=True)
		if c != "VL_PATRIM_LIQ"
	]
	_patch_download(monkeypatch, _csv(list_cols, [list_row]))

	with pytest.raises(ContractError):
		InfTrimestralFipReader(date_ref=REF).read()
...
def test_read_persists_csv_when_path_raw_is_given(
	...
	InfTrimestralFipReader(date_ref=REF, path_raw=path_raw).read()

	assert (path_raw / f"inf_trimestral_fip_{YEAR}.csv").is_file()
...
def test_reader_follows_the_retry_policy_standard(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	assert isinstance(InfTrimestralFipReader._RETRY_POLICY, RetryPolicy)
	assert (
		InfTrimestralFipReader(date_ref=REF)._retry_policy is InfTrimestralFipReader._RETRY_POLICY
	)
	assert (
		InfTrimestralFipReader(date_ref=REF, retry_policy=cls_custom)._retry_policy is cls_custom
	)
...
def test_read_rejects_wrong_argument_type(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	with pytest.raises(TypeError):
		InfTrimestralFipReader(date_ref=REF).read(int_timeout_s="nope")
