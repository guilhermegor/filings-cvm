"""Unit tests for the two AUDITOR/CAD registry readers (`cad_auditor.zip`).

One parameterized module covers both members: a single in-memory fixture ZIP holds the `pf` and
`pj` CSVs, and each reader is asserted to select **its own** member, coerce its date column, and
...
``tests/fixtures/cad_auditor/`` — the pinned oracle. Mock the single I/O boundary
...
from filings_cvm._internal.config.contracts import CAD_AUDITOR_PF, CAD_AUDITOR_PJ
from filings_cvm._internal.config.ports.ingestion_reader import IngestionReader
from filings_cvm._internal.utils.tabular_reader import ContractError, FileContract
from filings_cvm.ingestion.auditor import AuditorPfReader, AuditorPjReader
...
PATH_FIXTURES = Path(__file__).resolve().parents[1] / "fixtures" / "cad_auditor"
...
ALL_READERS: tuple[type[IngestionReader], ...] = (AuditorPfReader, AuditorPjReader)
...
SECTIONS: tuple[tuple[str, type[IngestionReader], FileContract], ...] = (
	("pf", AuditorPfReader, CAD_AUDITOR_PF),
	("pj", AuditorPjReader, CAD_AUDITOR_PJ),
)
...
def _patch_download(monkeypatch: pytest.MonkeyPatch, payload: bytes) -> None:
	...
	monkeypatch.setattr(
		"filings_cvm.ingestion.auditor.cad._base_auditor_reader.download_file",
		_fake_download,
	)
...
def test_contract_matches_the_published_header(
	...
	cls : type[IngestionReader]
		The reader whose contract is pinned (unused here beyond ``ids``; kept for symmetry).
	contract : FileContract
		The contract under test.
	"""
	str_line = (PATH_FIXTURES / f"cad_auditor_{str_section}_header.csv").read_text("utf-8").strip()
...
@pytest.mark.parametrize("cls", ALL_READERS, ids=lambda c: c.__name__)
def test_read_coerces_date_column(
	cls: type[IngestionReader], monkeypatch: pytest.MonkeyPatch
) -> None:
	"""``DT_INI_SIT`` becomes a pure ``date`` object.
...
def test_pf_contract_declares_no_cnpj_column() -> None:
	...
	assert CAD_AUDITOR_PF.tuple_cnpj_cols == ()
	assert "CNPJ" not in CAD_AUDITOR_PF.tuple_required
...
def test_read_raises_contract_error_on_missing_required_column(
	...
	list_cols = [c for c in AuditorPjReader._CONTRACT.tuple_required if c != "DENOM_SOCIAL"]
	dict_members[AuditorPjReader._MEMBER] = (
		";".join(list_cols) + "\n" + ";".join("x" for _ in list_cols) + "\n"
	)
	_patch_download(monkeypatch, _zip_bytes(dict_members))

	with pytest.raises(ContractError):
		AuditorPjReader().read()
...
def test_read_raises_value_error_when_member_absent(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	"""
	dict_members = _all_members()
	del dict_members[AuditorPjReader._MEMBER]
	_patch_download(monkeypatch, _zip_bytes(dict_members))

	with pytest.raises(ValueError, match="cad_auditor_pj.csv"):
		AuditorPjReader().read()
...
def test_read_persists_whole_archive_when_path_raw_is_given(
	...
	AuditorPfReader(path_raw=path_raw).read()

	assert (path_raw / "cad_auditor.zip").is_file()
...
def test_read_rejects_wrong_argument_type(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	with pytest.raises(TypeError):
		AuditorPfReader().read(int_timeout_s="nope")
