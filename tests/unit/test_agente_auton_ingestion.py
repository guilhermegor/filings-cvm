"""Unit tests for the two AGENTE_AUTON/CAD registry readers (`cad_agente_auton.zip`).
...
``tests/fixtures/cad_agente_auton/`` — the pinned oracle. Mock the single I/O boundary
...
from filings_cvm._internal.config.contracts import CAD_AGENTE_AUTON_PF, CAD_AGENTE_AUTON_PJ
from filings_cvm._internal.config.ports.ingestion_reader import IngestionReader
from filings_cvm._internal.utils.tabular_reader import ContractError, FileContract
from filings_cvm.ingestion.agente_auton import AgenteAutonPfReader, AgenteAutonPjReader
...
PATH_FIXTURES = Path(__file__).resolve().parents[1] / "fixtures" / "cad_agente_auton"
...
ALL_READERS: tuple[type[IngestionReader], ...] = (AgenteAutonPfReader, AgenteAutonPjReader)
...
SECTIONS: tuple[tuple[str, type[IngestionReader], FileContract], ...] = (
	("pf", AgenteAutonPfReader, CAD_AGENTE_AUTON_PF),
	("pj", AgenteAutonPjReader, CAD_AGENTE_AUTON_PJ),
)
...
def _patch_download(monkeypatch: pytest.MonkeyPatch, payload: bytes) -> None:
	...
	monkeypatch.setattr(
		"filings_cvm.ingestion.agente_auton.cad._base_agente_auton_reader.download_file",
		_fake_download,
	)
...
def test_contract_matches_the_published_header(
	...
	"""
	str_line = (
		(PATH_FIXTURES / f"cad_agente_auton_{str_section}_header.csv").read_text("utf-8").strip()
	)
...
def test_pf_contract_declares_no_cnpj_column() -> None:
	...
	assert CAD_AGENTE_AUTON_PF.tuple_cnpj_cols == ()
	assert "CNPJ" not in CAD_AGENTE_AUTON_PF.tuple_required
...
def test_read_raises_contract_error_on_missing_required_column(
	...
	list_cols = [c for c in AgenteAutonPjReader._CONTRACT.tuple_required if c != "DENOM_SOCIAL"]
	dict_members[AgenteAutonPjReader._MEMBER] = (
		";".join(list_cols) + "\n" + ";".join("x" for _ in list_cols) + "\n"
	)
	_patch_download(monkeypatch, _zip_bytes(dict_members))

	with pytest.raises(ContractError):
		AgenteAutonPjReader().read()
...
def test_read_raises_value_error_when_member_absent(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	"""
	dict_members = _all_members()
	del dict_members[AgenteAutonPjReader._MEMBER]
	_patch_download(monkeypatch, _zip_bytes(dict_members))

	with pytest.raises(ValueError, match="cad_agente_auton_pj.csv"):
		AgenteAutonPjReader().read()
...
def test_read_persists_whole_archive_when_path_raw_is_given(
	...
	AgenteAutonPfReader(path_raw=path_raw).read()

	assert (path_raw / "cad_agente_auton.zip").is_file()
...
def test_read_rejects_wrong_argument_type(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	with pytest.raises(TypeError):
		AgenteAutonPfReader().read(int_timeout_s="nope")
