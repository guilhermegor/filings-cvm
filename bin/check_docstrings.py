...
def _canonical_type_tokens(text: str) -> list:
	...
	# A bracketed Optional annotation means the value may be None, so emit that token
	# before the brackets are flattened to separators and the word is dropped.
...
def compare_types(hint: object, doc: str) -> bool:
	...
	# generic arguments, saying "list of dict" for a nested mapping, and it omits the
	# optional-None arm. Accept when the documented tokens are a subset of the
