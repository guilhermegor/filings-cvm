...
def render_comment(
	...
	if bool_auto_merge:
		str_merge = (
			"🤖 **Auto-merge ativado** — o GitHub funde sozinho assim que os checks obrigatórios "
			"ficarem verdes."
		)
	elif str_risk not in AUTO_MERGEABLE:
		str_merge = (
			f"👤 **Revisão humana obrigatória** — classe `{str_risk}` nunca é auto-fundida "
			"(um diff mínimo em `src/` pode quebrar um contrato sem falhar um único teste)."
		)
	else:
		str_merge = (
			f"💤 **Auto-merge disponível** — adicione o rótulo `{OPT_IN_LABEL}` para que o GitHub "
			"funda sozinho quando o gate ficar verde."
		)
	...
	return "\n".join(
		[
			...
			*list_rows,
			"",
			f"**Classe de risco:** `{str_risk}` · **Tamanho:** `{str_size}`",
			"",
			str_merge,
			"",
			"<sub>Comentário gerado pelo Quality Gate (`bin/pr_gate.py`). "
...
def _axes_from_checks(list_check_runs: list[dict]) -> list[tuple[str, str, str]]:
	...
	"""
	dict_axes = {
		"Testes (3 SOs)": "Run Automated Tests",
		"Documentação (build)": "build",
		"Segurança (CodeQL)": "CodeQL",
	}
	...
	for str_label, str_prefix in dict_axes.items():
		...
		if not list_matching:
			list_out.append((str_label, "⏳", "aguardando"))
			continue
		if any(c["status"] != "completed" for c in list_matching):
			list_out.append((str_label, "⏳", "em execução"))
			...
			list_out.append((str_label, "❌", f"{len(list_int_failed)} check(s) falhando"))
	return list_out
