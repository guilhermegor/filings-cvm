"""Shared base for the CVM COORD_OFERTA/CAD (offering-coordinator registry) ingestion readers.

`cad_coord_oferta.zip` ships **two members** — `cad_coord_oferta.csv` (the coordinator registry)
and `cad_coord_oferta_resp.csv` (its responsible-officer table). ⚠️ Like the INTERMED sibling of
this mould — and unlike AUDITOR / AGENTE_FIDUC / INVNR — the two members are **not a `pf`/`pj`
split**; they are two related tables of the same registry, both keyed by the coordinator's `CNPJ`.
...
`CoordOfertaReader` / `CoordOfertaRespReader` adapters, never this class. Each concrete reader is a
...
# CVM open-data offering-coordinator-registry snapshot ZIP, shared by both readers. Fixed URL: CVM
# overwrites this file in place.
_URL = "https://dados.cvm.gov.br/dados/COORD_OFERTA/CAD/DADOS/cad_coord_oferta.zip"

_ZIP_FILENAME = "cad_coord_oferta.zip"
...
class _BaseCoordOfertaReader(IngestionReader):
	"""Private base for the two COORD_OFERTA/CAD registry readers.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``cad_coord_oferta.zip`` and every CSV
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		-------
		pd.DataFrame
			The registry member — one row per registered coordinator (or officer). **No grain is
			asserted.**
		...
		self._cls_logger.log_message(
			f"Downloading COORD_OFERTA/CAD ({self._LABEL}) from {self._str_url}", "info"
		)
		dict_dtypes = {
			str_col: "str"
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from COORD_OFERTA/CAD", "info"
		)
