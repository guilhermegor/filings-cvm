"""Shared base for the CVM CONSULTOR_VLMOB/CAD (securities-consultant registry) readers.

`cad_consultor_vlmob.zip` ships **five members** — `cad_consultor_vlmob_pf.csv` and
`cad_consultor_vlmob_pj.csv` (the natural- and legal-person consultants) plus
`cad_consultor_vlmob_diretor.csv`, `cad_consultor_vlmob_resp.csv` and
`cad_consultor_vlmob_socios.csv` (the consultants' directors, responsible officers and partners).
It shares ADM_CART's five-member shape; the members differ only in their columns, so the download →
...
`ConsultorVlmobRespReader` / `ConsultorVlmobSociosReader` adapters, never this class. Each
concrete reader is a thin subclass that sets four class attributes — the member filename, its
...
# CVM open-data securities-consultant-registry snapshot ZIP, shared by all five readers. Fixed URL:
# CVM overwrites this file in place.
_URL = "https://dados.cvm.gov.br/dados/CONSULTOR_VLMOB/CAD/DADOS/cad_consultor_vlmob.zip"

_ZIP_FILENAME = "cad_consultor_vlmob.zip"
...
class _BaseConsultorVlmobReader(IngestionReader):
	"""Private base for the five CONSULTOR_VLMOB/CAD registry readers.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``cad_consultor_vlmob.zip`` and every CSV
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		-------
		pd.DataFrame
			The registry member — one row per consultant (or director/officer/partner). **No grain
			is asserted.**
		...
		self._cls_logger.log_message(
			f"Downloading CONSULTOR_VLMOB/CAD ({self._LABEL}) from {self._str_url}", "info"
		)
		dict_dtypes = {
			str_col: "str"
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from CONSULTOR_VLMOB/CAD", "info"
		)
