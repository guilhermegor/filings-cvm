"""CVM Informe Trimestral FIP — ingestion (leitura) reader.

Downloads the CVM open-data **quarterly report of Fundos de Investimento em Participações**
(``inf_trimestral_fip_AAAA.csv``, dataset ``FIP/DOC/INF_TRIMESTRAL``) and returns it as a typed,
contract-validated :class:`pandas.DataFrame`.

One row per fund per competency quarter, carrying the fund's net worth and quota position
(``VL_PATRIM_LIQ``, ``QT_COTA``, ``VL_PATRIM_COTA``), the committed / subscribed / paid-in capital,
the subscriber breakdown by investor category (``NR_COTST_SUBSCR_*`` holders and
``PR_COTA_SUBSCR_*`` percentages) and the per-class quota fields.

Two shape notes, both reflected below:

- The dump is **partitioned by year** — ``inf_trimestral_fip_2023.csv`` — so this reader's
  ``date_ref`` selects the *year* (only ``date_ref.year`` is read). The series runs 2010–2023;
  from 2024 the report became quadrimestral (:class:`InfQuadrimestralFipReader`).
- It is a **plain CSV, not a ZIP** — the downloaded file is read directly, like the DFIN index.
...
_BASE_URL = (
	"https://dados.cvm.gov.br/dados/FIP/DOC/INF_TRIMESTRAL/DADOS/inf_trimestral_fip_{yyyy}.csv"
)
...
_DTYPES: dict[str, str] = {
	str_col: "str" for str_col in INF_TRIMESTRAL_FIP.tuple_required if str_col not in _DATE_COLS
}
...
class InfTrimestralFipReader(IngestionReader):
	"""Read the CVM Informe Trimestral FIP open-data dump into a typed DataFrame.

	Concrete :class:`IngestionReader` for the yearly ``inf_trimestral_fip`` CSV — the pre-RCVM 175
	quarterly FIP report, one row per fund per competency quarter.
	...
	def __init__(
		...
		date_ref : datetime.date, optional
			...
			series ends in 2023 (from 2024 the report is quadrimestral), so pass a year in
			2010–2023 for data.
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``inf_trimestral_fip_AAAA.csv`` for a
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		seam, which enforces the :data:`INF_TRIMESTRAL_FIP` contract (all 54 columns, with a
		coercible ``CNPJ_FUNDO``) before applying the declared types. ``DT_COMPTC`` becomes a pure
		``date``; every other column — including the money and quota amounts — is exact source
		text.
		...
		-------
		pd.DataFrame
			One row per fund per competency quarter. **No unique key is asserted:** a fund reports
			once per quarter, and multiple share classes repeat the fund's CNPJ across rows.
		...
		ContractError
			If the CSV violates the :data:`INF_TRIMESTRAL_FIP` contract.
		"""
		str_year = self._date_ref.strftime("%Y")
		self._cls_logger.log_message(
			f"Downloading Informe Trimestral FIP from {self._str_url}", "info"
		)
		with raw_workspace(self._path_raw) as path_dir:
			path_csv = download_file(
				self._str_url,
				path_dir / f"inf_trimestral_fip_{str_year}.csv",
				int_timeout_s,
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} Informe Trimestral FIP rows from {str_year}", "info"
		)
		return stamp_provenance(df_, self._str_url, INF_TRIMESTRAL_FIP, str_content_hash)
