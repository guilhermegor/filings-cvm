"""CVM ITR CIA_ABERTA (demonstração do resultado (individual)) — ingestion (leitura) reader.
...
class ItrCiaAbertaDreIndReader(_BaseItrReader):
	"""Read this ITR CIA_ABERTA member into a typed DataFrame.

	Covers `demonstração do resultado (individual)`.
	...
	_MEMBER_STEM: ClassVar[str] = "itr_cia_aberta_DRE_ind"
	_CONTRACT: ClassVar[FileContract] = ITR_CIA_ABERTA_DRE_IND
	_DATE_COLS: ClassVar[tuple[str, ...]] = (
		"DT_REFER",
		"DT_INI_EXERC",
		"DT_FIM_EXERC",
	)
	_LABEL: ClassVar[str] = "demonstração do resultado (individual)"
