"""CVM ITR CIA_ABERTA (mutações do patrimônio líquido (individual)) — ingestion (leitura) reader.
...
class ItrCiaAbertaDmplIndReader(_BaseItrReader):
	"""Read this ITR CIA_ABERTA member into a typed DataFrame.

	Covers `mutações do patrimônio líquido (individual)`.
	...
	_MEMBER_STEM: ClassVar[str] = "itr_cia_aberta_DMPL_ind"
	_CONTRACT: ClassVar[FileContract] = ITR_CIA_ABERTA_DMPL_IND
	_DATE_COLS: ClassVar[tuple[str, ...]] = (
		"DT_REFER",
		"DT_INI_EXERC",
		"DT_FIM_EXERC",
	)
	_LABEL: ClassVar[str] = "mutações do patrimônio líquido (individual)"
