"""Shared base for the CVM CIA_ABERTA/DOC/FRE (*Formulário de Referência*) ingestion readers.

`fre_cia_aberta_AAAA.zip` is the **largest dataset in this portal**: 36 members and ~131k rows in
2025. It is implemented in four themed slices — index+capital, administração/pessoas, diversidade,
and remuneração/valores mobiliários — all sharing this base. They differ only in their columns and
date columns, so the download → unzip → select-member → read logic lives here once.

This is a **private** base (leading underscore, its own file): consumers import the concrete
`Fre*Reader` adapters, never this class. Each concrete reader sets four class attributes — the
member stem, its `FileContract`, its date columns and a log label — and inherits the rest.
...
⚠️ **Nothing is shared by assumption across members.** The index uses `CNPJ_CIA` / `DT_REFER` while
the satellites use `CNPJ_Companhia` / `Data_Referencia`, and the dataset as a whole uses **six**
different CNPJ column names. Each subclass therefore declares its own date columns, and each
contract its own CNPJ columns.

⚠️ **Money and counts stay exact text.** `Valor_Capital`, `Quantidade_*`, `Percentual_*` and the
`Remuneracao_*` columns of later slices are typed `str`, never a binary float — a float destroys
the published digits irreversibly and silently (`bin/check_dtypes.py` enforces it). Convert to
`Decimal` downstream.
"""
...
# CVM open-data **yearly** dump; ``{yyyy}`` is the reference year. Shared by every FRE reader.
_BASE_URL = "https://dados.cvm.gov.br/dados/CIA_ABERTA/DOC/FRE/DADOS/fre_cia_aberta_{yyyy}.zip"
...
class _BaseFreReader(IngestionReader):
	"""Private base for every CIA_ABERTA/DOC/FRE reader.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``fre_cia_aberta_AAAA.zip`` and the CSVs
			...
			FRE reader downloads the same archive, so a directory written by one serves the rest.
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		self._cls_logger.log_message(
			f"Downloading FRE CIA_ABERTA ({self._LABEL}) from {self._str_url}", "info"
		)
		...
		with raw_workspace(self._path_raw) as path_dir:
			path_zip = download_file(
				self._str_url,
				path_dir / f"fre_cia_aberta_{str_year}.zip",
				int_timeout_s,
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from FRE CIA_ABERTA {str_year}", "info"
		)
