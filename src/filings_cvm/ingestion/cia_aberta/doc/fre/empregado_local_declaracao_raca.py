"""CVM FRE CIA_ABERTA (raça declarada (empregados por local)) — ingestion (leitura) reader.

Race/colour counts declared for employees, grouped by **region** (`Local`).
...
Same seven buckets as the administrator member, but grouped by region rather than by
governance organ.
...
class FreCiaAbertaEmpregadoLocalDeclaracaoRacaReader(_BaseFreReader):
	"""Read this FRE CIA_ABERTA diversidade member into a typed DataFrame.

	Covers `raça declarada (empregados por local)`.
	...
	_MEMBER_STEM: ClassVar[str] = "fre_cia_aberta_empregado_local_declaracao_raca"
	_CONTRACT: ClassVar[FileContract] = FRE_CIA_ABERTA_EMPREGADO_LOCAL_DECLARACAO_RACA
	_DATE_COLS: ClassVar[tuple[str, ...]] = ("Data_Referencia",)
	_LABEL: ClassVar[str] = "raça declarada (empregados por local)"
