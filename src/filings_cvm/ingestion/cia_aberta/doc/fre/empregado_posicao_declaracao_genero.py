"""CVM FRE CIA_ABERTA (gênero declarado (empregados por posição)) — ingestion (leitura) reader.

Gender counts declared for employees, grouped by job **position** (`Posicao`).
...
class FreCiaAbertaEmpregadoPosicaoDeclaracaoGeneroReader(_BaseFreReader):
	"""Read this FRE CIA_ABERTA diversidade member into a typed DataFrame.

	Covers `gênero declarado (empregados por posição)`.
	...
	_MEMBER_STEM: ClassVar[str] = "fre_cia_aberta_empregado_posicao_declaracao_genero"
	_CONTRACT: ClassVar[FileContract] = FRE_CIA_ABERTA_EMPREGADO_POSICAO_DECLARACAO_GENERO
	_DATE_COLS: ClassVar[tuple[str, ...]] = ("Data_Referencia",)
	_LABEL: ClassVar[str] = "gênero declarado (empregados por posição)"
