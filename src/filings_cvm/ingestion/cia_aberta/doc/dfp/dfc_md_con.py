"""CVM DFP CIA_ABERTA (fluxo de caixa, método direto (consolidado)) — ingestion (leitura) reader.
...
class DfpCiaAbertaDfcMdConReader(_BaseDfpReader):
	"""Read this DFP CIA_ABERTA member into a typed DataFrame.

	Covers `fluxo de caixa, método direto (consolidado)`.
	...
	_MEMBER_STEM: ClassVar[str] = "dfp_cia_aberta_DFC_MD_con"
	_CONTRACT: ClassVar[FileContract] = DFP_CIA_ABERTA_DFC_MD_CON
	_DATE_COLS: ClassVar[tuple[str, ...]] = (
		"DT_REFER",
		"DT_INI_EXERC",
		"DT_FIM_EXERC",
	)
	_LABEL: ClassVar[str] = "fluxo de caixa, método direto (consolidado)"
