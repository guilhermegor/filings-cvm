"""Shared base for the CVM CIA_ABERTA/DOC/DFP (*Demonstrações Financeiras Padronizadas*) readers.

`dfp_cia_aberta_AAAA.zip` holds **19 members and ~1,17 million rows** in 2025: the filing index,
eight statement types each published in a *consolidado* and an *individual* variant, plus the share
composition and the auditor's opinion. They differ only in their columns and date columns, so the
...
`DfpCiaAberta*Reader` adapters, never this class. Each concrete reader sets four class attributes —
...
# CVM open-data **yearly** dump; ``{yyyy}`` is the reference year. Shared by every DFP reader.
_BASE_URL = "https://dados.cvm.gov.br/dados/CIA_ABERTA/DOC/DFP/DADOS/dfp_cia_aberta_{yyyy}.zip"
...
class _BaseDfpReader(IngestionReader):
	"""Private base for every CIA_ABERTA/DOC/DFP reader.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``dfp_cia_aberta_AAAA.zip`` and the CSVs
			...
			DFP reader downloads the same archive, so a directory written by one serves the rest.
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		self._cls_logger.log_message(
			f"Downloading DFP CIA_ABERTA ({self._LABEL}) from {self._str_url}", "info"
		)
		...
		with raw_workspace(self._path_raw) as path_dir:
			path_zip = download_file(
				self._str_url,
				path_dir / f"dfp_cia_aberta_{str_year}.zip",
				int_timeout_s,
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from DFP CIA_ABERTA {str_year}", "info"
		)
