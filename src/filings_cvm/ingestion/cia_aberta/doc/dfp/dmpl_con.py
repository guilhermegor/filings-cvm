"""CVM DFP CIA_ABERTA (mutações do patrimônio líquido (consolidado)) — ingestion (leitura) reader.
...
class DfpCiaAbertaDmplConReader(_BaseDfpReader):
	"""Read this DFP CIA_ABERTA member into a typed DataFrame.

	Covers `mutações do patrimônio líquido (consolidado)`.
	...
	_MEMBER_STEM: ClassVar[str] = "dfp_cia_aberta_DMPL_con"
	_CONTRACT: ClassVar[FileContract] = DFP_CIA_ABERTA_DMPL_CON
	_DATE_COLS: ClassVar[tuple[str, ...]] = (
		"DT_REFER",
		"DT_INI_EXERC",
		"DT_FIM_EXERC",
	)
	_LABEL: ClassVar[str] = "mutações do patrimônio líquido (consolidado)"
