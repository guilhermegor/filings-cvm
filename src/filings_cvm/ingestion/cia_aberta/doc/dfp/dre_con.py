"""CVM DFP CIA_ABERTA (demonstração do resultado (consolidado)) — ingestion (leitura) reader.
...
class DfpCiaAbertaDreConReader(_BaseDfpReader):
	"""Read this DFP CIA_ABERTA member into a typed DataFrame.

	Covers `demonstração do resultado (consolidado)`.
	...
	_MEMBER_STEM: ClassVar[str] = "dfp_cia_aberta_DRE_con"
	_CONTRACT: ClassVar[FileContract] = DFP_CIA_ABERTA_DRE_CON
	_DATE_COLS: ClassVar[tuple[str, ...]] = (
		"DT_REFER",
		"DT_INI_EXERC",
		"DT_FIM_EXERC",
	)
	_LABEL: ClassVar[str] = "demonstração do resultado (consolidado)"
