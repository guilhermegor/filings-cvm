"""CVM Cadastro de Companhias Abertas — ingestion (leitura) reader.

Downloads the CVM open-data registry **snapshot** of the publicly held companies registered with
the CVM (``cad_cia_aberta.csv``, dataset ``CIA_ABERTA/CAD``) and returns it as a typed,
contract-validated :class:`pandas.DataFrame`. **Inaugurates the ``cia_aberta/`` portal root** — the
last and largest root of the portal sweep (issue #41); this reader covers only the ``CAD``
registry, with the seven ``DOC`` financial-statement datasets and ``EVENTOS`` landing as their own
readers.

Like ``cad_fi.csv`` / ``cad_cia_estrang.csv``, it is a bare CSV at a **fixed URL** that CVM
...
from filings_cvm._internal.config.contracts.cad_cia_aberta import CAD_CIA_ABERTA
...
_URL = "https://dados.cvm.gov.br/dados/CIA_ABERTA/CAD/DADOS/cad_cia_aberta.csv"

_FILENAME = "cad_cia_aberta.csv"
...
_DTYPES: dict[str, str] = {
	str_col: "str" for str_col in CAD_CIA_ABERTA.tuple_required if str_col not in _DATE_COLS
}
...
class CadastroCiaAbertaReader(IngestionReader):
	"""Read the CVM Cadastro de Companhias Abertas registry snapshot into a typed DataFrame.

	Concrete :class:`IngestionReader` for ``cad_cia_aberta.csv``. Takes no reference date: the
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``cad_cia_aberta.csv`` for a datalake's
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		seam, which enforces the :data:`CAD_CIA_ABERTA` contract (all 47 columns plus coercible
		``CNPJ_CIA`` / ``CNPJ_AUDITOR``) before applying the declared types. The seven ``DT_*``
		columns become ``datetime.date`` (or ``NaT``); others are exact source text.
		...
		-------
		pd.DataFrame
			One row per registered publicly held company, 47 columns.
		...
		ContractError
			If the CSV violates the :data:`CAD_CIA_ABERTA` contract.
		"""
		self._cls_logger.log_message(
			f"Downloading Cadastro de Companhias Abertas from {self._str_url}", "info"
		)
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} Cadastro de Companhias Abertas rows", "info"
		)
		return stamp_provenance(df_, self._str_url, CAD_CIA_ABERTA, str_content_hash)
