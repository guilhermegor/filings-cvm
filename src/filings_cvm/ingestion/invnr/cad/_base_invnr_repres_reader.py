"""Shared base for the CVM INVNR/CAD (non-resident-investor representative registry) readers.

`cad_invnr_repres.zip` ships **two members** — `cad_invnr_repres_pf.csv` (natural-person
representatives) and `cad_invnr_repres_pj.csv` (legal-person representatives). Both are
current-state registry snapshots that differ only in their columns, so the download → unzip →
select-member → read logic lives here once rather than being repeated in each public reader.

This is a **private** base (leading underscore, its own file): consumers import the concrete
`InvnrRepresPfReader` / `InvnrRepresPjReader` adapters, never this class. Each concrete reader is a
thin subclass that sets four class attributes — the member filename, its `FileContract`, its date
...
# CVM open-data non-resident-investor-representative registry snapshot ZIP, shared by both readers.
# Fixed URL: CVM overwrites this file in place.
_URL = "https://dados.cvm.gov.br/dados/INVNR/CAD/DADOS/cad_invnr_repres.zip"

_ZIP_FILENAME = "cad_invnr_repres.zip"
...
class _BaseInvnrRepresReader(IngestionReader):
	"""Private base for the two INVNR/CAD registry readers.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``cad_invnr_repres.zip`` and every CSV
			...
			discarded. CVM overwrites the file in place, so a persisted snapshot is the only record
			of what the registry said that day.
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		``date`` objects; every other column is exact source text — including ``CEP``, ``TEL`` and
		``FAX``, which the CVM META declares ``numeric`` but which are identifiers, not quantities.
		...
		-------
		pd.DataFrame
			The registry member — one row per registered representative. **No grain is asserted.**
		...
		self._cls_logger.log_message(
			f"Downloading INVNR/CAD ({self._LABEL}) from {self._str_url}", "info"
		)
		dict_dtypes = {
			str_col: "str"
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from INVNR/CAD", "info"
		)
