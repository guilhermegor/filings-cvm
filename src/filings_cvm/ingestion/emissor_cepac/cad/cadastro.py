"""CVM Cadastro de Emissor CEPAC — ingestion (leitura) reader.

Downloads the CVM open-data registry **snapshot** of CEPAC issuers (``cad_emissor_cepac.csv``,
dataset ``EMISSOR_CEPAC/CAD``) and returns it as a typed, contract-validated
:class:`pandas.DataFrame`. Inaugurates the ``emissor_cepac/`` portal root.

CEPAC (Certificados de Potencial Adicional de Construção) are issued by **municipalities** running
an urban operation, so this registry is tiny (a handful of rows). Like ``cad_fi.csv``, it is a bare
CSV at a **fixed URL** that CVM overwrites in place — hence there is **no** ``date_ref``, and a
``path_raw`` snapshot is the only record of what the registry said on the day it was fetched (it
cannot be re-fetched).

``DT_REG`` / ``DT_CANCEL`` / ``DT_INI_SIT`` are coerced to pure ``date`` (blanks become ``NaT``);
every other column is exact source text. Network and CSV parsing go through this library's own
``_internal.utils`` seams, so the single I/O boundary is :func:`download_file`.
"""
...
_URL = "https://dados.cvm.gov.br/dados/EMISSOR_CEPAC/CAD/DADOS/cad_emissor_cepac.csv"

_FILENAME = "cad_emissor_cepac.csv"

# Coerced to pure ``date`` objects; blanks (an active issuer has no ``DT_CANCEL``) become ``NaT``.
...
_DTYPES: dict[str, str] = {
	str_col: "str" for str_col in CAD_EMISSOR_CEPAC.tuple_required if str_col not in _DATE_COLS
}
...
class CadastroEmissorCepacReader(IngestionReader):
	"""Read the CVM Cadastro de Emissor CEPAC registry snapshot into a typed DataFrame.

	Concrete :class:`IngestionReader` for ``cad_emissor_cepac.csv``. Takes no reference date: the
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``cad_emissor_cepac.csv`` for a datalake's
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		seam, which enforces the :data:`CAD_EMISSOR_CEPAC` contract (all 20 columns plus a
		coercible ``CNPJ``) before applying the declared types. ``DT_REG`` / ``DT_CANCEL`` /
		``DT_INI_SIT`` become ``datetime.date`` (or ``NaT``); others are exact source text.
		...
		-------
		pd.DataFrame
			One row per registered CEPAC issuer, 20 columns.
		...
		ContractError
			If the CSV violates the :data:`CAD_EMISSOR_CEPAC` contract.
		"""
		self._cls_logger.log_message(
			f"Downloading Cadastro de Emissor CEPAC from {self._str_url}", "info"
		)
		...
		self._cls_logger.log_message(f"Loaded {len(df_)} Cadastro de Emissor CEPAC rows", "info")
		return stamp_provenance(df_, self._str_url, CAD_EMISSOR_CEPAC, str_content_hash)
