"""Shared base for the CVM AGENTE_FIDUC/CAD (fiduciary-agent registry) ingestion readers.

`cad_agente_fiduc.zip` ships **two members** — `cad_agente_fiduc_pf.csv` (natural-person agents)
and `cad_agente_fiduc_pj.csv` (legal-person agents). Both are current-state registry snapshots that
...
`AgenteFiducPfReader` / `AgenteFiducPjReader` adapters, never this class. Each concrete reader is a
...
# CVM open-data fiduciary-agent-registry snapshot ZIP, shared by both readers. Fixed URL: CVM
# overwrites this file in place.
_URL = "https://dados.cvm.gov.br/dados/AGENTE_FIDUC/CAD/DADOS/cad_agente_fiduc.zip"

_ZIP_FILENAME = "cad_agente_fiduc.zip"
...
class _BaseAgenteFiducReader(IngestionReader):
	"""Private base for the two AGENTE_FIDUC/CAD registry readers.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``cad_agente_fiduc.zip`` and every CSV
			extracted from it — not just the member read — for a bronze layer. Created if
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		self._cls_logger.log_message(
			f"Downloading AGENTE_FIDUC/CAD ({self._LABEL}) from {self._str_url}", "info"
		)
		dict_dtypes = {
			str_col: "str"
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from AGENTE_FIDUC/CAD", "info"
		)
