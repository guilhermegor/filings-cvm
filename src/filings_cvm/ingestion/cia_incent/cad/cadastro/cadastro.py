"""CVM Cadastro de Companhias Incentivadas — ingestion (leitura) reader.

Downloads the CVM open-data registry **snapshot** of the incentivised companies registered with the
CVM (``cad_cia_incent.csv``, dataset ``CIA_INCENT/CAD``) and returns it as a typed,
contract-validated :class:`pandas.DataFrame`. Second slice of Wave 4 of the portal sweep (issue
#41), a sibling of the ``cia_estrang/`` root.
...
_URL = "https://dados.cvm.gov.br/dados/CIA_INCENT/CAD/DADOS/cad_cia_incent.csv"

_FILENAME = "cad_cia_incent.csv"

# Coerced to pure ``date`` objects; blanks become ``NaT``. ``MOTIVO_CANCEL`` is deliberately absent
# — it is free text, not a date. ``DT_INI_CATEG`` is all-empty in the current snapshot but is a
# date column by contract (declared as such in the META and populated in the CIA_ESTRANG sibling).
...
_DTYPES: dict[str, str] = {
	str_col: "str" for str_col in CAD_CIA_INCENT.tuple_required if str_col not in _DATE_COLS
}
...
class CadastroCiaIncentReader(IngestionReader):
	"""Read the CVM Cadastro de Companhias Incentivadas registry snapshot into a typed DataFrame.

	Concrete :class:`IngestionReader` for ``cad_cia_incent.csv``. Takes no reference date: the
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``cad_cia_incent.csv`` for a datalake's
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		seam, which enforces the :data:`CAD_CIA_INCENT` contract (all 47 columns plus coercible
		...
		-------
		pd.DataFrame
			One row per registered incentivised company, 47 columns.
		...
		ContractError
			If the CSV violates the :data:`CAD_CIA_INCENT` contract.
		"""
		self._cls_logger.log_message(
			f"Downloading Cadastro de Companhias Incentivadas from {self._str_url}", "info"
		)
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} Cadastro de Companhias Incentivadas rows", "info"
		)
		return stamp_provenance(df_, self._str_url, CAD_CIA_INCENT, str_content_hash)
