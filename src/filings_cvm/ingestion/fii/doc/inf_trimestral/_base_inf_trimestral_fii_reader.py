"""Shared base for the CVM *Informe Trimestral FII* (INF_TRIMESTRAL) ingestion readers.

``inf_trimestral_fii_AAAA.zip`` (dataset ``FII/DOC/INF_TRIMESTRAL``) ships **16 members** — the
tables of the FII quarterly report: cadastro (``geral``, ``complemento``), the asset portfolio
(``ativo``, ``ativo_garantia_rentabilidade``, ``direito``), the real-estate detail (``imovel`` and
its ``desempenho``/tenants/contracts, ``terreno``, acquisitions and alienations), plus
``rentabilidade_efetiva`` and ``resultado_contabil_financeiro``. Every member is keyed by
``CNPJ_Fundo_Classe`` + ``Data_Referencia`` + ``Versao``, differing only in which table it carries.

**Partitioned by YEAR, not by quarter — despite being the *quarterly* report.** One
``inf_trimestral_fii_2025.zip`` holds every quarter of 2025 (``Data_Referencia`` is the
quarter-end), and each member's file name embeds the year:
``inf_trimestral_fii_geral_2025.csv``. So this reader's ``date_ref`` selects the **year** — only
``date_ref.year`` is read, the month and day are ignored. Same trap as the FII monthly dump: filter
to a single quarter in the returned frame, on ``Data_Referencia``.

Rather than repeat the download → unzip → select-member → read logic in each of the 16 public
readers, that logic lives here once. This is a **private** base (leading underscore, its own file):
consumers import the 16 concrete ``InfTrimestralFii*Reader`` adapters, never this class. Each
concrete reader is a thin subclass that sets four class attributes — the member's stem, its
:class:`FileContract`, its date columns, and a human label — and inherits everything else.

All 16 readers download the *same* yearly archive, so a ``path_raw`` written by any one serves the
others. Most members are **long** (one row per asset / imóvel / contrato / inquilino /
transaction), so **no unique key is asserted**.
"""
...
# CVM open-data **yearly** dump; ``{yyyy}`` is the reference year. Shared by all 16 readers.
_BASE_URL = (
	"https://dados.cvm.gov.br/dados/FII/DOC/INF_TRIMESTRAL/DADOS/inf_trimestral_fii_{yyyy}.zip"
)

# Reader-owned default retry/backoff (CVM's open-data portal throttles under load): 5 attempts on
# a capped exponential schedule (~2, 4, 8, 10 s). Shared by the 16 readers, which inherit it via
...
class _BaseInfTrimestralFiiReader(IngestionReader):
	"""Private base for the 16 Informe Trimestral FII table readers.
	...
	# Per-reader default retry and backoff schedule, per the project-wide standard. The 16 readers
	...
	def __init__(
		...
		date_ref : datetime.date, optional
			...
			*quarterly* report. Defaults to today. The current year's file grows as quarters are
			published, so it is partial by definition — pass a past year for a complete one.
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``inf_trimestral_fii_AAAA.zip`` and every CSV
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		source text (``str``) — money, quantity, area and percentage fields keep full precision for
		a downstream ``Decimal`` cast.
		...
		-------
		pd.DataFrame
			The year's rows for this table — **all four quarters**. **No unique key is asserted:**
			most members are long (one row per asset / imóvel / contrato / inquilino /
			transaction). Filter on ``Data_Referencia`` for a single quarter.
		...
		self._cls_logger.log_message(
			f"Downloading Informe Trimestral FII ({self._LABEL}) from {self._str_url}", "info"
		)
		dict_dtypes = {
			str_col: "str"
		...
		with raw_workspace(self._path_raw) as path_dir:
			path_zip = download_file(
				self._str_url,
				path_dir / f"inf_trimestral_fii_{str_year}.zip",
				int_timeout_s,
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from Informe Trimestral FII {str_year}", "info"
		)
