"""CVM DFIN CRA — ingestion (leitura) reader.

Downloads the CVM open-data **index of CRA financial statements** (``dfin_cra_AAAA.csv``, dataset
``SECURIT/DOC/DFIN_CRA``) and returns it as a typed, contract-validated :class:`pandas.DataFrame`.
Inaugurates the ``securit/`` portal root.

**This is an index, not a statement.** Each row describes one financial statement a securitisation
issuer delivered for the CRA (Certificados de Recebíveis do Agronegócio) — its reference and
delivery dates, the auditor's opinion, and a ``Link_Download`` pointing at the document on fnet.
The reader **returns the link as text and does not follow it** (the ``DfinFiiReader``
principle — parse the artifact CVM publishes, nothing more).

Two shape notes:

- The dump is **partitioned by year** — ``dfin_cra_2025.csv`` — so this reader's ``date_ref``
...
_BASE_URL = "https://dados.cvm.gov.br/dados/SECURIT/DOC/DFIN_CRA/DADOS/dfin_cra_{yyyy}.csv"
...
_DTYPES: dict[str, str] = {
	str_col: "str" for str_col in DFIN_CRA.tuple_required if str_col not in _DATE_COLS
}
...
class DfinCraReader(IngestionReader):
	"""Read the CVM DFIN CRA open-data index into a typed DataFrame.

	Concrete :class:`IngestionReader` for the yearly ``dfin_cra`` index CSV — the list of financial
	statements delivered for the CRA, one row per document.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``dfin_cra_AAAA.csv`` for a datalake's bronze
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		seam, which enforces the :data:`DFIN_CRA` contract (all 9 columns plus a coercible
		...
		ContractError
			If the CSV violates the :data:`DFIN_CRA` contract.
		"""
		str_year = self._date_ref.strftime("%Y")
		self._cls_logger.log_message(f"Downloading DFIN CRA from {self._str_url}", "info")
		with raw_workspace(self._path_raw) as path_dir:
			path_csv = download_file(
				self._str_url,
				path_dir / f"dfin_cra_{str_year}.csv",
				int_timeout_s,
		...
		self._cls_logger.log_message(f"Loaded {len(df_)} DFIN CRA rows from {str_year}", "info")
		return stamp_provenance(df_, self._str_url, DFIN_CRA, str_content_hash)
