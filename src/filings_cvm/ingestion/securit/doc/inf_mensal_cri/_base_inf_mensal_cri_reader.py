"""Shared base for the CVM *Informe Mensal CRI* (INF_MENSAL_CRI) ingestion readers.
...
operations: geral, ativo/passivo, classe, créditos, carteira, modificação de carteira, desembolso,
fluxo de caixa, derivativos, cedente/devedor and responsável. Every member shares the four-column
key prefix (``CNPJ_Emissora``, ``Codigo_Identificacao_Certificado``, ``Data_Referencia``,
``Versao``) then carries its own section-specific columns. The 11 are a **uniform family** — same
artifact, same key — differing only in which section each carries.

Rather than repeat the download → unzip → select-member → read logic in each of the 11 public
readers, that logic lives here once. This is a **private** base (leading underscore, its own file):
consumers import the 11 concrete ``InfMensalCri*Reader`` adapters, never this class. Each concrete
...
``inf_mensal_cri_AAAA.zip`` and its members are ``inf_mensal_cri_<section>_AAAA.csv``, so
``date_ref`` selects the **year** (only ``date_ref.year`` is read) — the OTS/CRA pattern, not the
FIDC one. A year's file holds every month's rows, keyed by ``Data_Referencia``.

⚠️ **This is the CRI sibling of the CRA/OTS family, not a copy of it.** CRI is real-estate, not
agro or the generic residual: it shares seven section *names* but **has no
``direitos_creditorios``** (its receivables member is ``creditos``, 51 columns) and **adds four
members** absent from CRA/OTS — ``carteira``, ``carteira_modificacao``, ``creditos`` and
``responsavel``. See ``contracts/inf_mensal_cri.py``.

**Date columns are per-member, not shared** (hence ``_DATE_COLS`` on each subclass, with no default
here): every member has ``Data_Referencia``, ``classe`` adds ``Data_Vencimento`` and ``geral``
three more. Notably ``Indice_Subordinacao_Data_Base`` (in ``classe``) is **not** a date despite its
name — its real values are numeric (``0.00``) — so it is deliberately absent from that reader's
``_DATE_COLS``.

All 11 readers download the *same* yearly archive, so a ``path_raw`` written by any one serves the
...
# CVM open-data **yearly** dump; ``{yyyy}`` is the reference year. Shared by all 11 readers — the
# same archive holds every member.
_BASE_URL = (
	"https://dados.cvm.gov.br/dados/SECURIT/DOC/INF_MENSAL_CRI/DADOS/inf_mensal_cri_{yyyy}.zip"
)

# Reader-owned default retry/backoff for the CRI yearly dump, declared here as one source of truth
# and assigned by each of the 11 readers as its ``_RETRY_POLICY`` class attribute. CVM's open-data
...
class _BaseInfMensalCriReader(IngestionReader):
	"""Private base for the 11 Informe Mensal CRI section readers.
	...
	# Member-specific. Every section has the shared reference date, but classe carries one more
	# date column and geral three, so each subclass sets its own and the base declares no default.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``inf_mensal_cri_AAAA.zip`` and every CSV
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		columns become pure ``date`` objects; every other column is exact source text (``str``) —
		...
		self._cls_logger.log_message(
			f"Downloading Informe Mensal CRI ({self._LABEL}) from {self._str_url}", "info"
		)
		dict_dtypes = {
			str_col: "str"
		...
		with raw_workspace(self._path_raw) as path_dir:
			path_zip = download_file(
				self._str_url,
				path_dir / f"inf_mensal_cri_{str_year}.zip",
				int_timeout_s,
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from Informe Mensal CRI {str_year}", "info"
		)
