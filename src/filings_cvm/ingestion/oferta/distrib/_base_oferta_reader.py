"""Shared base for the CVM OFERTA/DISTRIB (securities-offering register) ingestion readers.

`oferta_distribuicao.zip` ships **two members**, one per regulatory regime —
`oferta_distribuicao.csv` (the historical register, pre-RCVM 160) and `oferta_resolucao_160.csv`
(the RCVM 160 offering requests). ⚠️ They are **not** a registry + satellite pair (unlike
COORD_OFERTA): they are two offering tables of different regimes with disjoint columns. Either way
...
`OfertaDistribuicaoReader` / `OfertaResolucao160Reader` adapters, never this class. Each concrete
reader is a thin subclass that sets four class attributes — the member filename, its
`FileContract`, its date columns, and a log label — and inherits everything else.

Unlike the partitioned document dumps this is a **current-state register** at a fixed URL (no
`AAAAMM` partition), so the readers take **no `date_ref`**, and CVM overwrites the file in place —
persist `path_raw` to keep a day's snapshot. Both readers download the *same* archive, so a
`path_raw` written by one serves the other. No grain is asserted.

The ISO `Data_*` columns are typed to pure `date`; every other column is exact source text — the
`Nr_*` / `Num_*` / `Qtd_*` / `Qtde_*` counts and the `Valor_*` / `Preco_*` monetary fields keep
CVM's exact decimal text for a downstream `Decimal` cast, and `resolucao_160`'s
`Data_deliberacao_aprovou_oferta` (a `DD/MM/YYYY` date the ISO-only coercion cannot parse) stays
`str`.
"""
...
# CVM open-data securities-offering-register snapshot ZIP, shared by both readers. Fixed URL: CVM
# overwrites this file in place.
_URL = "https://dados.cvm.gov.br/dados/OFERTA/DISTRIB/DADOS/oferta_distribuicao.zip"

_ZIP_FILENAME = "oferta_distribuicao.zip"
...
class _BaseOfertaReader(IngestionReader):
	"""Private base for the two OFERTA/DISTRIB register readers.
	...
	-------
	read(int_timeout_s)
		Download, unzip, and parse this reader's register member into a validated DataFrame.
	"""
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``oferta_distribuicao.zip`` and every CSV
			...
			record of what the register said that day.
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		"""Download, extract, and parse this reader's register member into a typed DataFrame.
		...
		:class:`FileContract` before applying the declared types. The declared ``Data_*`` columns
		become pure ``date`` objects; every other column is exact source text — the count and
		monetary fields keep CVM's exact decimal text for a downstream ``Decimal`` cast.
		...
		-------
		pd.DataFrame
			The register member — one row per registered offering (or RCVM 160 request). **No grain
			is asserted.**
		...
		self._cls_logger.log_message(
			f"Downloading OFERTA/DISTRIB ({self._LABEL}) from {self._str_url}", "info"
		)
		dict_dtypes = {
			str_col: "str"
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from OFERTA/DISTRIB", "info"
		)
