"""Shared base for the CVM AGENTE_AUTON/CAD (autonomous-agent registry) ingestion readers.

`cad_agente_auton.zip` ships **two members** — `cad_agente_auton_pf.csv` (natural-person agents)
and `cad_agente_auton_pj.csv` (legal-person agents). Both are current-state registry snapshots that
differ only in their columns, so the download → unzip → select-member → read logic lives here once
rather than being repeated in each public reader.

This is a **private** base (leading underscore, its own file): consumers import the concrete
`AgenteAutonPfReader` / `AgenteAutonPjReader` adapters, never this class. Each concrete reader is a
...
# CVM open-data autonomous-agent-registry snapshot ZIP, shared by both readers. Fixed URL: CVM
# overwrites this file in place.
_URL = "https://dados.cvm.gov.br/dados/AGENTE_AUTON/CAD/DADOS/cad_agente_auton.zip"

_ZIP_FILENAME = "cad_agente_auton.zip"
...
class _BaseAgenteAutonReader(IngestionReader):
	"""Private base for the two AGENTE_AUTON/CAD registry readers.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``cad_agente_auton.zip`` and every CSV
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		``date`` objects; every other column is exact source text.
		...
		-------
		pd.DataFrame
			The registry member — one row per registered agent. **No grain is asserted.**
		...
		self._cls_logger.log_message(
			f"Downloading AGENTE_AUTON/CAD ({self._LABEL}) from {self._str_url}", "info"
		)
		dict_dtypes = {
			str_col: "str"
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from AGENTE_AUTON/CAD", "info"
		)
