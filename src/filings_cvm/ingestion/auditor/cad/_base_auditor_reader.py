"""Shared base for the CVM AUDITOR/CAD (auditor registry) ingestion readers.

`cad_auditor.zip` ships **two members** — `cad_auditor_pf.csv` (natural-person auditors) and
`cad_auditor_pj.csv` (audit firms). Both are current-state registry snapshots that differ only in
their columns, so the download → unzip → select-member → read logic lives here once rather than
being repeated in each public reader.

This is a **private** base (leading underscore, its own file): consumers import the concrete
`AuditorPfReader` / `AuditorPjReader` adapters, never this class. Each concrete reader is a thin
...
`path_raw` to keep a day's snapshot. Both readers download the *same* archive, so a `path_raw`
written by one serves the other. No grain is asserted.
"""
...
# CVM open-data auditor-registry snapshot ZIP, shared by both readers. Fixed URL: CVM overwrites
# this file in place.
_URL = "https://dados.cvm.gov.br/dados/AUDITOR/CAD/DADOS/cad_auditor.zip"

_ZIP_FILENAME = "cad_auditor.zip"
...
class _BaseAuditorReader(IngestionReader):
	"""Private base for the two AUDITOR/CAD registry readers.
	...
	def __init__(
		...
		path_raw : pathlib.Path, optional
			Directory in which to **persist** the raw ``cad_auditor.zip`` and every CSV extracted
			from it — not just the member read — for a datalake's bronze layer. Created if absent.
	...
	def read(self, int_timeout_s: int = 60) -> pd.DataFrame:
		...
		:class:`FileContract` before applying the declared types. ``DT_INI_SIT`` becomes a pure
		``date``; every other column is exact source text.
		...
		-------
		pd.DataFrame
			The registry member — one row per registered auditor. **No grain is asserted.**
		...
		self._cls_logger.log_message(
			f"Downloading AUDITOR/CAD ({self._LABEL}) from {self._str_url}", "info"
		)
		dict_dtypes = {
			str_col: "str"
		...
		self._cls_logger.log_message(
			f"Loaded {len(df_)} {self._LABEL} rows from AUDITOR/CAD", "info"
		)
